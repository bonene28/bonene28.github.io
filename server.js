...
/v2/me uses the user's Pi access token.
...
await pool.query(  CREATE INDEX IF NOT EXISTS   idx_mining_member_status   ON mining_sessions(   member_id,   status   );  );

await pool.query(  CREATE INDEX IF NOT EXISTS   idx_ledger_member   ON amt_ledger(member_id);  );

await pool.query(  CREATE INDEX IF NOT EXISTS   idx_referral_referrer   ON referrals(referrer_member_id);  );

await pool.query(  CREATE INDEX IF NOT EXISTS   idx_security_owner   ON security_circle(owner_member_id);  );
...
Required by the current index.html.
...
app.listen(  
  ...
  () => {  
    ...
    console.log(  
      "KYC required for Testnet mining:",  
      "NO"  
    );  
